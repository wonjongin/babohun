...
if __name__ == "__main__":
    print("=== 미래 예측 환자수 기반 현실적 시뮬레이션 실행 (수정된 병상가동률) ===")
    target_year = 2025
    ...
    pd.DataFrame([metrics]).to_csv(os.path.join(result_dir, f'scenario1_result_realistic_fixed_{target_year}.csv'), index=False)
    # 평가 리포트 저장
    rows = []
    ...
    pd.DataFrame(rows).to_csv(os.path.join(result_dir, f'scenario1_evaluation_report_realistic_fixed_{target_year}.csv'), index=False) 