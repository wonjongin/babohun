...
class RealisticHospitalSimulation:
    def __init__(self, env, beds_capacity, total_specialists, hospital_patient_info, sim_time=8760, verbose=True):
        ...
        # 재원일수 추적 추가
        self.total_bed_days = {h: 0.0 for h in beds_capacity}
        # 각 병원별 환자 도착 프로세스 시작
        for hospital in hospital_patient_info:
            self.env.process(self.hospital_patient_arrival(hospital))

    def hospital_patient_arrival(self, hospital):
        """각 병원별로 환자가 시간에 따라 도착하는 프로세스"""
        info = self.hospital_patient_info[hospital]
        ...
        for day in range(365):
            # 해당 일의 시작 시간
            day_start = day * 24
            # 해당 일에 도착할 환자 수 (포아송 분포)
            num_inpatients = np.random.poisson(daily_inpatients)
            
            # 환자들을 해당 일에 분산해서 도착
            for i in range(num_inpatients):
                # 일 중 랜덤한 시간에 도착
                arrival_time = day_start + random.uniform(0, 24)
                # 현재 시간과 도착 시간의 차이를 계산
                delay = max(0, arrival_time - self.env.now)
        ...
        for day in range(250):
            ...
            for i in range(num_outpatients):
                # 외래 시간대(9-17시)에 도착
                arrival_time = day_start + random.uniform(9, 17)
                # 현재 시간과 도착 시간의 차이를 계산
                delay = max(0, arrival_time - self.env.now)
                yield self.env.timeout(delay)
        ...
        if patient_type == '입원':
            with self.beds[hospital].request() as bed_req:
                ...
                with self.specialists.request() as doc_req:
                    ...
                    # 재원일수 추가
                    self.total_bed_days[hospital] += treatment_time / 24  # 시간을 일로 변환
        else:
            with self.specialists.request() as doc_req:
                yield doc_req
                wait_time = self.env.now - arrival
...
# ---------------------------
# 5. 평가지표 계산 함수 (수정된 병상가동률)
# ---------------------------
def calculate_evaluation_metrics(sim, beds_capacity, total_specialists):
    total_beds = sum(beds_capacity.values())
    # 수정된 병상가동률 계산: 실제 재원일수 사용
    total_bed_days = sum(sim.total_bed_days.values())
    bed_utilization = (total_bed_days / (total_beds * 365)) * 100 if total_beds > 0 else 0
    ...
    # 평균재원일수 계산
    total_inpatients = sum(sim.inpatient_count.values())
    avg_length_of_stay = total_bed_days / total_inpatients if total_inpatients > 0 else 0
    ...
    metrics = {
        ...
        '평균재원일수': avg_length_of_stay,
        '총재원일수': total_bed_days
    }
    return metrics
...
if __name__ == "__main__":
    print("=== 미래 예측 환자수 기반 현실적 시뮬레이션 실행 (최종 수정) ===")
    target_year = 2025
    ...
    # 결과 저장
    result_dir = 'simulation_results_realistic_final'
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    pd.DataFrame([metrics]).to_csv(os.path.join(result_dir, f'scenario1_result_realistic_final_{target_year}.csv'), index=False)
    # 평가 리포트 저장
    rows = []
    ...
    pd.DataFrame(rows).to_csv(os.path.join(result_dir, f'scenario1_evaluation_report_realistic_final_{target_year}.csv'), index=False) 