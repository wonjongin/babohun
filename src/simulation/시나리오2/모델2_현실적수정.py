...
# ---------------------------
# 4. 현실적인 시뮬레이션 클래스 (입원/외래 분리, 모델2 개선)
# ---------------------------
class RealisticHospitalSim:
    def __init__(self, env, beds_capacity, specialists_capacity, hospital_patient_info, sim_time=8760, verbose=True):
        self.env = env
...
# 5. 평가지표 계산 함수
# ---------------------------
def calculate_evaluation_metrics(sim, beds_capacity, specialists_capacity):
    total_beds = sum(beds_capacity.values())
    total_specialists = sum(specialists_capacity.values())
...
if __name__ == "__main__":
    print("=== 미래 예측 환자수 기반 현실적 시뮬레이션(모델2) 실행 ===")
    target_year = 2025
    hospital_patient_info = get_hospital_patient_info(target_year=target_year, target_col='XGB예측')
    sim_time = 8760  # 1년
    specialists_capacity = {h: int(round(hospital_data[hospital_data['병원명'] == h][doc_columns].sum(axis=1).values[0])) for h in hospital_data['병원명'].unique()}
    env = simpy.Environment()
    sim = RealisticHospitalSim(env, beds_capacity, specialists_capacity, hospital_patient_info, sim_time=sim_time, verbose=False)
    env.run(until=sim_time)
    metrics = calculate_evaluation_metrics(sim, beds_capacity, specialists_capacity)
    ...
    # 결과 저장
    result_dir = 'simulation_results_model2_realistic'
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    pd.DataFrame([metrics]).to_csv(os.path.join(result_dir, f'scenario2_result_realistic_{target_year}.csv'), index=False)
    # 평가 리포트 저장
    rows = []
    ...
    pd.DataFrame(rows).to_csv(os.path.join(result_dir, f'scenario2_evaluation_report_realistic_{target_year}.csv'), index=False) 