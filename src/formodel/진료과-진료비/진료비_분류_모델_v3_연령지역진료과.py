...
def load_and_preprocess_data():
    ...
    # 연령지역진료과 예측 데이터 로드
    age_region_dept_df = pd.read_csv('model_results_연령지역_진료과/Stacking_prediction_results_detailed.csv')
    
    print(f"기본 데이터 shape: {df.shape}")
    print(f"연령지역진료과 예측 데이터 shape: {age_region_dept_df.shape}")
    
    # 연령지역진료과 예측 데이터에서 필요한 컬럼만 선택
    age_region_features = age_region_dept_df[['year_num', 'age_num', 'is_major_city', 'is_inpatient', 
                                            ...
                                            'confidence', 'sample_weight']].copy()
    
    # 중복 제거 (동일한 상병코드에 대해 하나의 예측값만 사용)
    age_region_features = age_region_features.drop_duplicates(subset='disease_group', keep='first')
    
    # 상병코드와 매핑
    df['disease_group'] = df['상병코드'].str[:3]
    
    # 병합
    df_merged = df.merge(age_region_features, on='disease_group', how='left')
    ...
    # 범주형 변수 처리
    categorical_columns = ['지역', 'age_group', '구분', 'region_age']
    for col in categorical_columns:
        if col in df_merged.columns:
            df_merged[col] = df_merged[col].fillna('Unknown')
            le = LabelEncoder()
            df_merged[f'{col}_encoded'] = le.fit_transform(df_merged[col].astype(str))
...
def prepare_features(df):
    ...
    # 연령 관련 컬럼 찾기 (실제 데이터의 연령 컬럼들)
    age_columns = [col for col in df.columns if any(age in col for age in ['59이하', '60-64', '65-69', '70-79', '80-89', '90이상'])]
    feature_columns.extend(age_columns)
    
    # 지역 관련 컬럼 찾기 (실제 6개 지역)
    region_columns = [col for col in df.columns if any(region in col for region in ['서울', '인천', '부산', '대구', '대전', '광주'])]
    feature_columns.extend(region_columns)
    
    # 진료과 더미 변수
    dept_dummy_columns = [col for col in df.columns if col.startswith('진료과_')]
    feature_columns.extend(dept_dummy_columns)
    
    # 기타 유용한 컬럼들
    useful_columns = ['년도', '연인원', '진료비(천원)']
    for col in useful_columns:
        if col in df.columns:
            feature_columns.append(col)
    
    # 숫자형 컬럼 중 타겟 제외한 것들 추가
    numeric_columns = df.select_dtypes(include=[np.number]).columns.tolist()
    if 'cost_bin_5' in numeric_columns:
        numeric_columns.remove('cost_bin_5')
    feature_columns.extend(numeric_columns)
    ...
    print(f"발견된 피쳐 컬럼: {available_features}")
    
    if len(available_features) == 0:
        print("Warning: 사용 가능한 피쳐가 없습니다. 기본 숫자형 컬럼을 사용합니다.")
        available_features = [col for col in df.select_dtypes(include=[np.number]).columns if col != 'cost_bin_5']
    ...
    print(f"사용된 피쳐 수: {len(available_features)}")
    print(f"피쳐 목록: {available_features}")
    
    return X, y, mask  # mask를 반환하여 원본 데이터와의 매핑을 유지
...
def save_prediction_results(X_test, y_test, sampling_results, ens_results, tune_results, output_dir, original_df=None):
    ...
    if original_df is not None:
        ...
        # 인코딩 값들도 저장
        encoded_columns = ['disease_encoded', 'dept_encoded', 'age_group_encoded', 'region_encoded']
...
def main():
    print("=== 진료비 예측 모델 v3 (연령지역진료과 피쳐 + 샘플링/앙상블/튜닝) ===")
    ...
    # 결과 저장
    output_dir = "model_results_v3_연령지역진료과_확장"
