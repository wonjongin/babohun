...
# 5-Fold CV & early stopping callback
cv_res = lgb.cv(
    params       = lgb_params,
    train_set    = dtrain,
...
# 테스트셋 성능
pred_log = final_lgb.predict(X_te_rg, num_iteration=best_round)
...
example_reg  = "부산"
print("\n=== 예시 예측 (LightGBM) ===")
cost_lgb = predict_cost(example_code, example_reg, final_lgb, feat_cols)
print(f"LightGBM 예상 진료비: {np.expm1(cost_lgb):,.0f} 천원")
