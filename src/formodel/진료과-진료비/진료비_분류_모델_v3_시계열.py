...
def train_ensemble(X_train, X_test, y_train, y_test, scaler=None):
    ...
    # 추가 성능 지표
    test_precision = precision_score(y_test, y_pred_weighted, average='weighted', zero_division='warn')
    test_recall = recall_score(y_test, y_pred_weighted, average='weighted', zero_division='warn')
    try:
        if len(np.unique(y_test)) > 2:
            test_auc = roc_auc_score(y_test, weighted_predictions, multi_class='ovr', average='weighted')
        else:
            test_auc = roc_auc_score(y_test, weighted_predictions[:, 1])
    except:
        test_auc = 0.0
    ...
    weighted_result = {
        ...
        'cv_f1_std': 0.0,  # 단순화
        'cv_acc_mean': sum(model_weights.values()) / len(model_weights),  # 평균 가중치
        'cv_acc_std': 0.0,  # 단순화
    ...
    for samp_name, res in sampling_results.items():
        for model_name, r in res.items():
            summary_stats.append({
                ...
                'Test_F1_Weighted': r['test_f1_score'],
                'Test_F1_Macro': r.get('test_f1_macro', 0),
                'Test_Precision_Weighted': r.get('test_precision', 0),
                'Test_Recall_Weighted': r.get('test_recall', 0),
                'Test_Precision_Macro': r.get('test_precision_macro', 0),
                'Test_Recall_Macro': r.get('test_recall_macro', 0),
