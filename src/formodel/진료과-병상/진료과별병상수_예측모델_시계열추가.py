...
df_pred = pd.read_csv("analysis_data/병원별_진료과별_시계열예측결과_개선.csv")
...
results_df.to_csv("hospital_bed_model_comparison_metrics_gridcv.csv", encoding="utf-8-sig", index=False)
print(results_df)

for name, pred_df in pred_dfs.items():
    pred_df.to_csv(f"hospital_bed_prediction_results_{name}_gridcv.csv", encoding="utf-8-sig", index=False)
