...
print(f"0이 아닌 환자 수 비율: {(df['전체환자수'] > 0).mean():.2%}")
...
for 병원 in df['병원'].unique():
    for 진료과 in df['진료과'].unique():
        ...
        # 시계열 데이터 준비
        ts_data = subset.set_index('연도')['전체환자수']
...
if future_predictions:
    ...
    future_df.to_csv('analysis_data/병원별_진료과별_입원_미래3년_예측결과.csv', index=False, encoding='utf-8')
    print(f"미래 예측 저장 완료: analysis_data/병원별_진료과별_입원_미래3년_예측결과.csv")
    
    # 미래 예측 요약
    print("\n=== 미래 예측 요약 ===")
    for model in ['ARIMA예측', 'RF예측', 'XGB예측']:
        values = future_df[model].values
        print(f"{model}:")
        print(f"  평균 예측값: {np.mean(values):.0f}")
        print(f"  최대 예측값: {np.max(values):.0f}")
        print(f"  최소 예측값: {np.min(values):.0f}")
        print(f"  예측 가능한 조합 수: {len(future_df)}")
else:
...
if results:
    ...
    result_df.to_csv('analysis_data/병원별_진료과별_입원_시계열예측결과.csv', index=False, encoding='utf-8')
    print('저장 완료: analysis_data/병원별_진료과별_입원_시계열예측결과.csv')
