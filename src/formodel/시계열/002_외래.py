...
# 데이터 불러오기
df = pd.read_csv('new_merged_data/연도별진료과별외래환자수.csv')
...
for 병원 in df['병원'].unique():
    for 진료과 in df['진료과'].unique():
        ts = df[(df['병원'] == 병원) & (df['진료과'] == 진료과)]
        ts = ts.groupby('연도')['전체환자수'].sum()
...
result_df.to_csv('analysis_data/병원별_진료과별_외래_시계열예측결과.csv', index=False, encoding='utf-8')
print('저장 완료: analysis_data/병원별_진료과별_외래_시계열예측결과.csv')
...
'''
저장 완료: analysis_data/병원별_진료과별_외래_시계열예측결과.csv
ARIMA 전체 평균 RMSE: 4852.03
RF 전체 평균 RMSE: 4558.45
XGB 전체 평균 RMSE: 4265.43
'''