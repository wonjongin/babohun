...
print("=== 진료비 분배 최적화 모델 ===")
print("📊 진료과별 효율적이고 적절한 진료비 분배 시스템")
print()
...
# 같은 상병코드의 다른 병원 데이터로 인원수 비례하여 추정
for idx, row in df_cost[df_cost['진료비(천원)'].isna()].iterrows():
    같은_상병 = df_cost[
    ...
    if len(같은_상병) > 0:
        ...
        df_cost.loc[idx, '진료비(천원)'] = 추정_진료비

print(f"✅ 빈 진료비 값 처리 완료")
print()
...
# 최적화 문제 정의
prob = LpProblem("진료비_분배_최적화", LpMaximize)
...
fig.suptitle('진료비 분배 최적화 결과 분석', fontsize=16, fontweight='bold')
